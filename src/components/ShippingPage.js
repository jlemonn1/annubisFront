...
const ShippingPage = () => {
  ...
  useEffect(() => {
    fetch(`https://200.234.229.234:8080/api/carts/email/${email}`)
      .then(response => {
        if (!response.ok) {
  ...
  const handlePayment = () => {
    ...
    const checkoutUrl = `https://200.234.229.234:8080/api/checkout/create-checkout-session?cartEmail=${email}&shippingCost=${shippingCost}&mobileNumber=${mobileNumber}&fullAddress=${formattedAddress}&typeShipping=${shippingOption}&discountName=${discountCode}`;
  ...
  const applyDiscount = () => {
    fetch(`https://200.234.229.234:8080/api/discounts/nombre/${discountCode}`)
      .then(response => {
        if (!response.ok) {
