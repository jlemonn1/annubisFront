...
const ShippingPage = () => {
  ...
  const handlePayment = () => {
    ...
    const checkoutUrl = `/api/checkout/create-checkout-session?cartEmail=${email}&shippingCost=${shippingCost}&mobileNumber=${mobileNumber}&fullAddress=${formattedAddress}&typeShipping=${shippingOption}&discountName=${discountCode}`;
  ...
  const applyDiscount = () => {
    fetch(`/api/discounts/nombre/${discountCode}`)
      .then(response => {
        if (!response.ok) {
